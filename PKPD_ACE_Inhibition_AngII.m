...
function output = PKPD_ACE_Inhibition_AngII(coefficients, drugdose,...
    ...
    n_Hill,AngI_conc_t0,AngII_conc_t0,Renin_conc_t0,diacid_conc_t0,...               %% CE AngI Value
...
baseline_prod_AngI = baseline_cons_AngII+k_degr_AngI*AngI_conc_t0;    %% CE AngI Value
...
if tfinal_dosing(end) == 0
    ...
    output = [tfinal_dosing,diacid_conc_t0,AngII_conc_t0*Mw_AngII*1000/10^6,...
        AngI_conc_t0*Mw_AngI*1000/10^6,0,Renin_conc_t0*Mw_Renin*1000/10^6,drug_conc_t0]; %% CE AngI Value
else
    %% call the ODE solver
    ...
    [t,conc] = ode45(@(t,conc) ODE(t,conc,drugdose,ke_diacid,...
            VF_diacid,ka_diacid,feedback_capacity,baseline_prod_AngI,...    %% CE AngI Value
...
output = [t,diacid_conc,AngII_conc,AngI_conc,Inhibition,Renin_conc,drug_conc];  %% CE AngI Value
...
function d_conc_dt = ODE(t,conc,drugdose,ke_diacid,VF_diacid,ka_diacid,...
    feedback_capacity,baseline_prod_AngI,baseline_cons_AngII,...            %% CE AngI Value
    VmaxoverKm,k_cat_Renin,k_feedback,C50,n_Hill,tau,tfinal_dosing,AngII_conc_t0,...
    Renin_conc_t0,baseline_prod_Renin,k_degr_Renin,k_degr_AngI,k_cons_AngII)  %% CE AngI Value
